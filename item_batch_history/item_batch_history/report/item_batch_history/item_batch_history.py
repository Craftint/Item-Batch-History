		...
		for wh in sorted(iwb_map[item]):
			for batch in sorted(iwb_map[item][wh]):
				...
				if qty_dict.opening_qty or qty_dict.in_qty or qty_dict.out_qty or qty_dict.bal_qty:
					data.append([item, item_map[item]["item_name"], item_map[item]["description"],wh, frappe.db.get_value('Batch', batch, 'expiry_date'),
...
def get_columns(filters):
	...
	columns =  [_("Batch") + ":Link/Batch:100"] +  [_("Item") + ":Link/Item:100"] + [_("Item Name") + "::150"] + \
			   [_("Warehouse") + ":Link/Warehouse:120"] + [_("Expiry Date") + ":Date:90"] + \
...
def get_stock_ledger_entries(filters):
	...
	return frappe.db.sql("""
		select Sle.batch_no, Sle.item_code, Sle.warehouse, Sle.posting_date, bh.expiry_date, bh.item, Sle.actual_qty
		from `tabStock Ledger Entry` as Sle inner join `tabBatch` as bh on bh.item = Sle.item_code
